import type { PlanInputs, PlanResult } from "../types.js";
 ...
 */
export function auraGreeting(ctx: ChatContext): ChatReply {
...
export function auraReply(message: string, ctx: ChatContext): ChatReply {
  ...
  if (has(m, "what is", "explain", "mean", "define", "how does")) {
    if (has(m, "xirr")) return { text: `XIRR is your real annualized return once the timing of every contribution is counted — the honest "what did my money actually earn" number.`, quickReplies: DEFAULT_QUICK_REPLIES };
    if (has(m, "sip")) return { text: `A SIP is a fixed amount you invest every month, automatically. Small and steady beats lump-sum timing for most people.`, quickReplies: DEFAULT_QUICK_REPLIES };
    if (has(m, "equity")) return { text: `Equity = owning a slice of companies. Highest long-run growth, but it swings. Best when your goal is years away.`, quickReplies: DEFAULT_QUICK_REPLIES };
    if (has(m, "debt") || has(m, "bond")) return { text: `Debt & bonds are loans you make to govts/companies for steady interest. Lower returns, much smoother — your portfolio's shock absorber.`, quickReplies: DEFAULT_QUICK_REPLIES };
