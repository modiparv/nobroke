import type { Step } from "../../lib/onboarding.js";
...
function setAnswer(field: string, value: string | number): void {
  if (PROFILE_FIELDS.has(field)) {
    setState({ profile: { ...getState().profile, [field]: value } as Profile });
  } else {
...
export function mountOnboarding(root: HTMLElement): void {
  ...
  function continueBtn(label = "Continue", enabled = true, onClick?: () => void): HTMLButtonElement {
    const b = el("button", { class: "btn-primary btn-lg ob-continue", type: "button", text: label }) as HTMLButtonElement;
    b.disabled = !enabled;
    if (onClick) b.addEventListener("click", onClick);
    return b;
  }

  function buildIntro(step: Step): void {
    body.append(
      el("div", { class: "ob-step ob-intro" }, [
  ...
  function buildGoals(step: Step): void {
    ...
    function refresh(): void {
      ...
      for (const c of chips) c.classList.toggle("selected", sel.includes(c.getAttribute("data-id") ?? ""));
    }
    ...
    for (const g of GOALS) {
      ...
      chip.addEventListener("click", () => {
        ...
        if (sel.includes(g.id)) sel = sel.filter((x) => x !== g.id);
        else if (sel.length < 3) sel.push(g.id);
        else return;
  ...
  function buildSingle(step: Step): void {
    const grid = el("div", { class: "ob-option-grid" });
    const current = PROFILE_FIELDS.has(step.field ?? "")
      ? String((getState().profile as unknown as Record<string, unknown>)[step.field ?? ""] ?? "")
  ...
  function sliderDefault(step: Step): number {
  ...
  function buildSlider(step: Step): void {
    const field = step.field ?? "";
    const p = getState().profile as unknown as Record<string, number>;
    let value = touched.has(field) ? p[field] ?? 0 : sliderDefault(step);
    if (!touched.has(field)) setAnswer(field, value); // seed the default
    ...
    body.append(
      el("div", { class: "ob-step ob-slider-step" }, [
        el("h1", { class: "ob-title", text: step.title }),
        step.subtitle ? el("p", { class: "ob-subtitle", text: step.subtitle }) : el("span", {}),
  ...
  function buildOutro(step: Step): void {
    body.append(
      el("div", { class: "ob-step ob-intro" }, [
  ...
  function render(): void {
    const s = getState();
    if (s.onboardingStepIndex === lastIndex) return;
