...
export function mountAggregationPanel(root: HTMLElement): void {
  clear(root);
  ...
  for (const p of AGGREGATION_PROVIDERS) {
    ...
    const btn = el("button", { class: "btn-ghost agg-btn", type: "button", text: "Connect" }) as HTMLButtonElement;
