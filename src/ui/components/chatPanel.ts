import type { ChatContext } from "../../lib/chat.js";
...
export function mountChatPanel(root: HTMLElement): void {
  ...
  const input = el("input", { class: "chat-input", type: "text", placeholder: "Ask AURA anything…", "aria-label": "Message AURA" }) as HTMLInputElement;
  ...
  function render(): void {
    ...
    if (chat.length === lastLen) return;
    lastLen = chat.length;

    clear(messages);
    for (const m of chat) {
      messages.append(
        el("div", { class: "chat-msg chat-" + m.role }, [el("span", { class: "chat-bubble", text: m.text })]),
      );
    }
