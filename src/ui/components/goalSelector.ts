...
export function mountGoalSelector(root: HTMLElement): void {
  ...
  function addChip(id: string, emoji: string, name: string): void {
  ...
  clear(root);
  for (const g of GOALS) addChip(g.id, g.emoji, g.name);
  addChip(CUSTOM_GOAL_ID, "✨", "Custom");

  function sync(): void {
    const { goalId } = getState();
    for (const chip of chips) chip.classList.toggle("active", chip.getAttribute("data-id") === goalId);
  }
