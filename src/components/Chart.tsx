  ...
  return (
    <div className="mt-3">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="Projected growth">
        ...
        {xTicks.map((m, i) => (
          <text key={i} x={x(m)} y={PAD.t + PH + 22} textAnchor="middle" fontSize="11" fill={C.muted} fontFamily={mono}>
            {formatYears(m / 12)}
          </text>
        ))}
        <path d={area} fill="url(#g-area)" />
        <path d={investedPath} fill="none" stroke={C.invested} strokeWidth="1.6" strokeDasharray="4 4" />
        <path d={valuePath} fill="none" stroke={C.brand} strokeWidth="2.6" strokeLinejoin="round" strokeLinecap="round" />
        <line x1={PAD.l} y1={ty} x2={PAD.l + PW} y2={ty} stroke={C.ink} strokeWidth="1.3" strokeDasharray="2 4" />
        <text x={PAD.l + PW} y={ty - 8} textAnchor="end" fontSize="11" fontWeight="600" fill={C.ink}>
          Target {formatINR(r.requiredCorpus)}
        </text>
        <circle cx={x(maxX)} cy={y(r.projectedCorpus)} r="5" fill={r.onTrack ? C.positive : C.ink} />
      </svg>
      <div className="mt-2 flex flex-wrap gap-4 font-mono text-[11px] uppercase tracking-wide text-muted">
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block h-1 w-3.5 rounded" style={{ background: C.brand }} /> Projected
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block h-1 w-3.5 rounded" style={{ background: C.invested }} /> Invested
        </span>
        <span className="inline-flex items-center gap-1.5">
