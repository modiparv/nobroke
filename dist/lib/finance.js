...
export function normalizedWeights(alloc) {
 ...
 */
export function projectionSeries(inputs, annualReturn) {
    const months = Math.max(1, Math.round(inputs.horizonYears * 12));
    const rm = annualReturn / 12;
    const series = [{ month: 0, year: 0, invested: inputs.currentSavings, value: inputs.currentSavings }];
...
export function requiredCorpus(targetToday, inflation, horizonYears) {
...
export function requiredSip(target, currentSavings, annualReturn, horizonYears) {
...
export function buildCashFlows(inputs, projectedCorpus) {
    const months = Math.max(1, Math.round(inputs.horizonYears * 12));
    const start = new Date(2025, 0, 1);
    const flows = [{ amount: -(inputs.currentSavings + inputs.monthlySip), date: new Date(start) }];
    for (let m = 1; m < months; m++) {
