...
export function renderChart(container, r) {
    const series = r.series;
    const maxX = Math.max(1, r.months);
    const maxY = Math.max(1, Math.max(r.projectedCorpus, r.requiredCorpus) * 1.12);
    const xOf = (month) => PAD.left + (month / maxX) * plotW;
    const yOf = (val) => PAD.top + plotH - (val / maxY) * plotH;
    const linePath = (key) => series.map((p, i) => `${i === 0 ? "M" : "L"}${xOf(p.month).toFixed(1)},${yOf(p[key]).toFixed(1)}`).join(" ");
  ...
  <div class="chart-legend">
    ...
    const capture = qs(".c-capture", container);
    const hover = qs(".c-hover", container);
    const hLine = qs(".c-hover-line", container);
    const hDot = qs(".c-hover-dot", container);
    const tip = qs(".chart-tooltip", container);
