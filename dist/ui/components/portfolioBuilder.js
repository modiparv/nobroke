...
import { currentGoal, subscribe, updateCurrentGoal } from "../state.js";
...
function balance() {
    ...
    const diff = 100 - sum;
    if (diff !== 0)
        a[[...w].sort((x, y) => y.weight - x.weight)[0].id] += diff;
...
export function mountPortfolioBuilder(root) {
    ...
    for (const cat of CATEGORIES) {
        ...
        group.append(el("div", { class: "lib-group-head" }, [
    ...
    function rebuildList() {
        ...
        for (const id of ordered) {
            ...
            const slider = el("input", { class: "prow-slider", type: "range", min: "0", max: "100", step: "1", "aria-label": p.name + " weight" });
    ...
    function updateVisuals() {
        ...
        for (const cat of CATEGORIES) {
            ...
            if (w <= 0)
                continue;
            catLegend.append(el("span", { class: "cat-legend-item" }, [
        ...
        for (const [id, slider] of rowSliders) {
            ...
            if (pct)
                pct.textContent = `${Math.round(total > 0 ? (raw / total) * 100 : 0)}%`;
        }
        ...
        for (const { key, btn } of modelButtons)
            btn.classList.toggle("active", g?.activeProfile === key);
    }
    function onState() {
        const g = currentGoal();
